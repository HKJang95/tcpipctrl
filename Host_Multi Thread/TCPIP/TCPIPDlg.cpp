...
void ManageThreadProc(CTCPIPDlg* pPrivate)
{
	pPrivate->commandQueueManager();
}

void ActionThreadProc(CTCPIPDlg* pPrivate)
{
	pPrivate->ActionControl(pPrivate->m_command, pPrivate->m_run);
}
	...
	, m_run(0)
{
	...
	m_hmanageTerminate = CreateEvent(NULL, true, false, _T("MANAGE_TERMINATE"));

	for (int i = 0; i < OP_MAX_VAL; i++)
	{
		m_hcommandThread[i] = NULL;
		m_IsThreadRunning[i] = FALSE;
		m_pBitmap[i] = NULL;
	}
...
BOOL CTCPIPDlg::OnInitDialog()
{
	...
	ResetEvent(m_hmanageTerminate);
	m_hmanageThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ManageThreadProc, this, 0, NULL);
	...
	else
	{
		if (m_IsCameraOpen)
		{
			for (int i = 0; i < OP_MAX_VAL; i++)
			{
				...
				{
					m_pGraphics->DrawImage(m_pBitmap[i], 0, 0, m_pCamera->m_camWidth, m_pCamera->m_camHeight);
					break;
				}
			}
		}
...
void CTCPIPDlg::OnDestroy()
{
	...
	if (m_hmanageThread != NULL)
	{
		SetEvent(m_hmanageTerminate);
...
void CTCPIPDlg::commandQueueManager()
{
...
void CTCPIPDlg::ActionControl(CString ReceiveMsg, int mynumber)
{
	m_IsThreadRunning[mynumber] = TRUE;
	...
	if (m_IsSerialOpen)
	{
		...
		{
			opLog(_T("라이트 켜기 실패!"));
			AddCompleteMsg(LIGHT_FAIL, &msg, brightStr);
			m_IsLightOn = FALSE;
		}
		else
		{
	...
	if (m_IsCameraOpen)
	{
		...
		{
			innerLogMsg.Format(_T("%f 노출 지정 완료."), expose);
			opLog(innerLogMsg);
		}
		else
		{
		...
		{
			AddCompleteMsg(GRAB_FAIL, &msg, brightStr);
	...
	if (!m_IsSerialOpen)
	{
		AddCompleteMsg(SERIAL_NOT_OPEN, &msg, brightStr);
		opLog(_T("시리얼 포트 Open되지 않음. 라이트 제어 불가."));
	}
	if (complete)
	{
		AddCompleteMsg(ACTION_SUCCESS, &msg, brightStr);
		opLog(_T("영상 촬영 완료."));
	}
...
BOOL CTCPIPDlg::LightControl(int brightness)
{
	...
	else
	{
		...
		{
			bright.Format(_T("0%d"), brightness);
		}
		else
		{
			bright.Format(_T("00%d"), brightness);
		}
	}
	sendASCII = (CString)(_T("CH1S") + bright);
	sendMsg = (CString)(m_stx + sendASCII + m_etx);
	m_Comm->Send(sendMsg);
	return TRUE;
}

void CTCPIPDlg::AddCompleteMsg(INT32 errcode, CString* errMsg, CString Bright)
{
	...
	{
	case ACTION_SUCCESS:
		strCompleteMsg.Format(_T("[SUCCESS]#") + Bright + _T("#") + exposetimeStr + _T("\n"));
		break;
...
BOOL CTCPIPDlg::RawToBmp(int mynumber)
{
	...
	else
	{
		...
		m_pBitmap[mynumber]->LockBits(&rc, 0, PixelFormat8bppIndexed, &bitmapdata);
		memcpy(bitmapdata.Scan0, m_pCamera->m_pImage, m_pCamera->m_bufferSize);
		m_pBitmap[mynumber]->UnlockBits(&bitmapdata);
		if (m_IsSerialOpen)
		{
		...
		int size = m_pBitmap[mynumber]->GetPaletteSize();
		pPalette = new ColorPalette[size];
		m_pBitmap[mynumber]->GetPalette(pPalette, size);
		for (unsigned int i = 0; i<pPalette->Count; i++)
		{
			pPalette->Entries[i] = Color::MakeARGB(255, i, i, i);
		}
		m_pBitmap[mynumber]->SetPalette(pPalette);
		delete[] pPalette;
...
BOOL CTCPIPDlg::BmpSave(int mynumber)
{
	time_t timer;
	struct tm *t = new tm;
	...
	auto timeauto = std::chrono::system_clock::now();
	auto milli = std::chrono::duration_cast<std::chrono::milliseconds>(timeauto.time_since_epoch());

	long long currentTimeMillis = milli.count();
	...
	cfname.Format(_T("%s\\[%d-%d-%d_%dh%dm%ds%dms][ip_%s_port_%d].bmp"), m_PathName,t->tm_year + 1900, t->tm_mon + 1,
		t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec, msc ,m_ReceiveIP, m_ReceivePort);
	_pfname = (TCHAR *)(LPCTSTR)cfname;

	CLSID clsid;
	CLSIDFromString(L"{557cf400-1a04-11d3-9a73-0000f81ef32e}", &clsid);

	m_pBitmap[mynumber]->Save(_pfname, &clsid, NULL);
...
void CTCPIPDlg::OnBnClickedCambtn()
{
	...
	else
	{
		if (m_pCamera != NULL)
		{
			...
			for (int i = 0; i < OP_MAX_VAL; i++)
			{
				m_pBitmap[i] = new Bitmap(m_pCamera->m_camWidth, m_pCamera->m_camHeight, PixelFormat8bppIndexed);
			}
			m_pGraphics->DrawImage(m_pBitmap[0], 0, 0, m_pCamera->m_camWidth, m_pCamera->m_camHeight);
		}
	}
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}
...
void CTCPIPDlg::opLog(CString ip, UINT uport, CString content)
{
	...
	temp.Format(_T("%s IP : %s PORT : %d 명령 : %s  수신됨"),timestr, ip, uport, content);
