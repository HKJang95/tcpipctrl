...
IMPLEMENT_DYNAMIC(SerialPort, CCmdTarget)
...
SerialPort::SerialPort(CString port, CString baudrate, CString parity, CString databit, CString stopbit)
{
	m_sComPort = port;
...
UINT CommThread(LPVOID lpData)
{
	...
	SerialPort* Comm = (SerialPort*)lpData;
...
BOOL SerialPort::Create(HWND hWnd)
{
	m_hWnd = hWnd;
...
BOOL SerialPort::Send(CString outbuf)
{
	BOOL		isSend = TRUE;
	DWORD       ErrorFlags;
	COMSTAT     ComStat;
...
int SerialPort::Receive(LPSTR inbuf, int len)
{
...
BEGIN_MESSAGE_MAP(SerialPort, CCmdTarget)
END_MESSAGE_MAP()



// SerialPort 메시지 처리기입니다.
